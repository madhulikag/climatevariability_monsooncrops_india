...
# Predicted rainfed grain yield calculated from the linear mixed-effects model
# Linear mixed effects model coefficients 

# Linear mixed effects model - actual yields and actual rainfall and temperature - rainfed yields
...
yield_climate <- merge(Harv_Irr_Yield, actual_tmax_rain1, by=c("Year", "Dist.Code"))
...
Maize_Rice_complete <- subset(yield_climate_nino34_iod, RiceIrrigation_Type=="Rainfed_Rice" & MaizeIrrigation_Type=="Rainfed_Maize")
Sorghum_Rice_complete <- subset(yield_climate_nino34_iod, RiceIrrigation_Type=="Rainfed_Rice" & SorghumIrrigation_Type=="Rainfed_Sorghum")
PearlMillet_Rice_complete <- subset(yield_climate_nino34_iod, RiceIrrigation_Type=="Rainfed_Rice" & PearlMilletIrrigation_Type=="Rainfed_PearlMillet")
FingerMillet_Rice_complete <- subset(yield_climate_nino34_iod, RiceIrrigation_Type=="Rainfed_Rice" & FingerMilletIrrigation_Type=="Rainfed_FingerMillet")
...
coefficients <- fixef(m1.1)
coefficients

# Create sequences of temperature and rainfall
temp_seq <- seq(min(Maize_Rice_wide_soil1$mean_actual_tmax), max(Maize_Rice_wide_soil1$mean_actual_tmax))
rain_seq <- seq(min(Maize_Rice_wide_soil1$total_actual_rain), max(Maize_Rice_wide_soil1$total_actual_rain))

# Predict yields using the polynomial terms
pred_yield_temp <- coefficients['poly(mean_actual_tmax, 2)1'] * poly(temp_seq, 2)[, 1] +
  coefficients['poly(mean_actual_tmax, 2)2'] * poly(temp_seq, 2)[, 2]

pred_yield_rain <- coefficients['poly(total_actual_rain, 2)1'] * poly(rain_seq, 2)[, 1] +
  coefficients['poly(total_actual_rain, 2)2'] * poly(rain_seq, 2)[, 2]

# Plotting yields versus temperature
ggplot() +
  geom_line(aes(x = temp_seq, y = pred_yield_temp), color = 'blue') +
  labs(x = 'Mean Actual Tmax', y = 'Predicted Rice Yield', title = 'Yield vs Temperature') +
  theme_minimal() +
  theme(
    axis.text.x = element_text(size = 16), # Increase font size of x-ticks
    axis.text.y = element_text(size = 16)  # Increase font size of y-ticks
  )

# Plotting yields versus rainfall
ggplot() +
  geom_line(aes(x = rain_seq, y = pred_yield_rain), color = 'green') +
  labs(x = 'Total Actual Rain', y = 'Predicted Rice Yield', title = 'Yield vs Rainfall') +
  theme_minimal() +
  theme(
    axis.text.x = element_text(size = 16), # Increase font size of x-ticks
    axis.text.y = element_text(size = 16)  # Increase font size of y-ticks
...
ggplot() +
  geom_line(aes(x = temp_seq, y = pred_yield_temp), color = 'blue') +
  labs(x = 'Mean Actual Tmax', y = 'Predicted Maize Yield', title = 'Yield vs Temperature') +
  theme_minimal() +
  theme(
    axis.text.x = element_text(size = 16), # Increase font size of x-ticks
    axis.text.y = element_text(size = 16)  # Increase font size of y-ticks
...
ggplot() +
  geom_line(aes(x = rain_seq, y = pred_yield_rain), color = 'green') +
  labs(x = 'Total Actual Rain', y = 'Predicted Maize Yield', title = 'Yield vs Rainfall') +
  theme_minimal() +
  theme(
    axis.text.x = element_text(size = 16), # Increase font size of x-ticks
    axis.text.y = element_text(size = 16)  # Increase font size of y-ticks
...
# Extract coefficients
coefficients <- fixef(m2.1)
coefficients

# Create sequences of temperature and rainfall
temp_seq <- seq(min(Sorghum_Rice_wide_soil1$mean_actual_tmax), max(Sorghum_Rice_wide_soil1$mean_actual_tmax))
rain_seq <- seq(min(Sorghum_Rice_wide_soil1$total_actual_rain), max(Sorghum_Rice_wide_soil1$total_actual_rain))
...
ggplot() +
  geom_line(aes(x = temp_seq, y = pred_yield_temp), color = 'blue') +
  labs(x = 'Mean Actual Tmax', y = 'Predicted Sorghum Yield', title = 'Yield vs Temperature') +
  theme_minimal() +
  theme(
    axis.text.x = element_text(size = 16), # Increase font size of x-ticks
    axis.text.y = element_text(size = 16)  # Increase font size of y-ticks
...
ggplot() +
  geom_line(aes(x = rain_seq, y = pred_yield_rain), color = 'green') +
  labs(x = 'Total Actual Rain', y = 'Predicted Sorghum Yield', title = 'Yield vs Rainfall') +
  theme_minimal() +
  theme(
    axis.text.x = element_text(size = 16), # Increase font size of x-ticks
    axis.text.y = element_text(size = 16)  # Increase font size of y-ticks
...
# Extract coefficients
coefficients <- fixef(m3.1)
coefficients

# Create sequences of temperature and rainfall
temp_seq <- seq(min(PearlMillet_Rice_wide_soil1$mean_actual_tmax), max(PearlMillet_Rice_wide_soil1$mean_actual_tmax))
rain_seq <- seq(min(PearlMillet_Rice_wide_soil1$total_actual_rain), max(PearlMillet_Rice_wide_soil1$total_actual_rain))
...
ggplot() +
  geom_line(aes(x = temp_seq, y = pred_yield_temp), color = 'blue') +
  labs(x = 'Mean Actual Tmax', y = 'Predicted Pearl Millet Yield', title = 'Yield vs Temperature') +
  theme_minimal() +
  theme(
    axis.text.x = element_text(size = 16), # Increase font size of x-ticks
    axis.text.y = element_text(size = 16)  # Increase font size of y-ticks
...
ggplot() +
  geom_line(aes(x = rain_seq, y = pred_yield_rain), color = 'green') +
  labs(x = 'Total Actual Rain', y = 'Predicted Pearl Millet Yield', title = 'Yield vs Rainfall') +
  theme_minimal() +
  theme(
    axis.text.x = element_text(size = 16), # Increase font size of x-ticks
    axis.text.y = element_text(size = 16)  # Increase font size of y-ticks
...
# Extract coefficients
coefficients <- fixef(m4.1)
coefficients

# Create sequences of temperature and rainfall
temp_seq <- seq(min(FingerMillet_Rice_wide_soil1$mean_actual_tmax), max(FingerMillet_Rice_wide_soil1$mean_actual_tmax))
rain_seq <- seq(min(FingerMillet_Rice_wide_soil1$total_actual_rain), max(FingerMillet_Rice_wide_soil1$total_actual_rain))
...
ggplot() +
  geom_line(aes(x = temp_seq, y = pred_yield_temp), color = 'blue') +
  labs(x = 'Mean Actual Tmax', y = 'Predicted Finger Millet Yield', title = 'Yield vs Temperature') +
  theme_minimal() +
  theme(
    axis.text.x = element_text(size = 16), # Increase font size of x-ticks
    axis.text.y = element_text(size = 16)  # Increase font size of y-ticks
...
ggplot() +
  geom_line(aes(x = rain_seq, y = pred_yield_rain), color = 'green') +
  labs(x = 'Total Actual Rain', y = 'Predicted Finger Millet Yield', title = 'Yield vs Rainfall') +
  theme_minimal() +
  theme(
    axis.text.x = element_text(size = 16), # Increase font size of x-ticks
    axis.text.y = element_text(size = 16)  # Increase font size of y-ticks
